 ...
 *    2. Emit 'reset' so server.ts closes all browser tabs.
...
export interface VMManagerEvents {
  ...
  spare: (status: SpareStatus) => void;
}

export type SpareStatus = 'none' | 'booting' | 'ready';

// ── TabSession event map ─────────────────────────────────────────────────────

export interface TabSessionEvents {
...
export class TabSession extends EventEmitter {
  ...
  constructor(id: string, vmManager: VMManager) {
    super();
    this.id  = id;
    this._vm = vmManager;
  }
      ...
      client.on('ready', () => {
        client.shell(
          ...
          (err, stream) => {
            ...
            (stream as ClientChannel & { stderr: NodeJS.ReadableStream }).stderr.on(
              'data',
              (data: Buffer) => {
                if (!this._closed) this.emit('data', data);
              }
            ...
            stream.on('error', (streamErr: Error) => {
              console.error(`[Tab ${this.id}] SSH stream error:`, streamErr.message);
            });
  ...
  resize(cols: number, rows: number): void {
    ...
    this._rows = rows;
    if (this._stream && !this._stream.destroyed) {
      (this._stream as ClientChannel & { setWindow: (rows: number, cols: number, height: number, width: number) => void })
        .setWindow(rows, cols, rows * 16, cols * 8);
    }
  }
  ...
  close(): void {
    ...
    this._closed = true;
    if (this._stream) {
      try { this._stream.close(); } catch (_) { /* ignore */ }
      this._stream = null;
    }
    if (this._client) {
      try { this._client.end(); } catch (_) { /* ignore */ }
      this._client = null;
    }
    this.emit('close');
  }
...
export interface VMInstanceOptions {
  label:   string;
  image:   string;
  port:    number;
  qemuBin: string;
...
type QemuOutputCallback = (inst: VMInstance, data: Buffer) => void;
type QemuExitCallback   = (inst: VMInstance, code: number | null, signal: NodeJS.Signals | null) => void;
...
export class VMInstance {
  ...
  constructor({ label, image, port, qemuBin, memory, cpus }: VMInstanceOptions) {
    this.label   = label;
    this.image   = image;
    this.port    = port;
  ...
  launch(onOutput: QemuOutputCallback, onExit: QemuExitCallback): void {
    ...
    try {
      execFileSync('cp', ['--sparse=always', config.baseImage, this.image]);
    } catch (_) {
    ...
    this.proc.stdout!.on('data', (d: Buffer) => onOutput(this, d));
    this.proc.stderr!.on('data', (d: Buffer) => process.stderr.write(`[QEMU:${this.label}] ${d}`));
  ...
  probeSsh(): Promise<boolean> {
    return new Promise((resolve) => {
      const client = new Client();
      const done = (r: boolean): void => { try { client.end(); } catch (_) { /* ignore */ } resolve(r); };
  ...
  async waitForSsh(abortFn: () => boolean): Promise<boolean> {
    ...
    while (!abortFn()) {
      ...
      await new Promise<void>((r) => setTimeout(r, config.sshRetryMs));
    }
    return false;
  }
}
...
export class VMManager extends EventEmitter {
  ...
  // The two VM slots
  private _active: VMInstance | null = null;   // VMInstance currently serving users
  private _spare:  VMInstance | null = null;   // VMInstance pre-booting in the background

  private _tabs:              Map<string, TabSession> = new Map();
  private _watchdogTimer:     ReturnType<typeof setInterval> | null = null;
  private _watchdogFailures   = 0;
   ...
   * and the public API (server.ts can call vm.reset() directly).
   */
  async reset(): Promise<void> {
    ...
    if (this._spare && this._spare.ready) {
      ...
      this._spare        = null;
      this.emit('spare', 'none' as SpareStatus);
  ...
  private _makeInstance(label: string, image: string, port: number): VMInstance {
  ...
  private async _bootSpare(): Promise<void> {
    if (!this._running) return;
    if (this._spare) return;  // already booting

    if (!fs.existsSync(config.baseImage)) return;

    // Pick the port AND image that the current active VM is NOT using.
    // After a failover the promoted-spare runs on sparePort/spareImage, so the
    // new spare must use sshPort/workImage — and vice-versa.
    const usedPort  = this._active ? this._active.port  : null;
    const usedImage = this._active ? this._active.image : null;
    const sparePort  = (usedPort  === config.sparePort)  ? config.sshPort   : config.sparePort;
    const spareImage = (usedImage === config.spareImage) ? config.workImage  : config.spareImage;

    console.log('[VM:spare] Starting warm-up…');
    this._spare = this._makeInstance('spare', spareImage, sparePort);
    this._spare.launch(
      (inst, d) => this._onQemuOutput(inst, d),
      (inst, code, signal) => this._onQemuExit(inst, code, signal)
    );
    this.emit('spare', 'booting' as SpareStatus);
    ...
    if (ok && this._spare) {
      ...
      this.emit('spare', 'ready' as SpareStatus);
    } else if (this._spare) {
  ...
  private _onQemuOutput(inst: VMInstance, d: Buffer): void {
    ...
    if (
      ...
      this._stopWatchdog();
      this.reset().catch((err: Error) => console.error('[VM] Fault-triggered reset failed:', err.message));
    }
  }

  /** Handle QEMU process exit for either VM instance. */
  private _onQemuExit(inst: VMInstance, code: number | null, signal: NodeJS.Signals | null): void {
    ...
    if (inst === this._spare) {
      ...
      this._spare = null;
      this.emit('spare', 'none' as SpareStatus);
      if (this._running && !this._resetInProgress) void this._bootSpare();
      return;
    }
    ...
    this._stopWatchdog();
    this.reset().catch((err: Error) => console.error('[VM] Exit-triggered reset failed:', err.message));
  }
  ...
  private _startWatchdog(): void {
    ...
    this._watchdogTimer = setInterval(async () => {
      ...
      if (this._watchdogFailures >= config.watchdogMaxFailures) {
        ...
        this._stopWatchdog();
        this.reset().catch((err: Error) => console.error('[VM] Watchdog reset failed:', err.message));
      }
